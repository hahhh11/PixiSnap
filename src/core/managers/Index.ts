import { destroyWaiting } from "./WaitingManager";
import { destroyToast } from "./ToastManager";
...
export { showToast } from "./ToastManager";

export * from "./WaitingManager";
/**
 * 展示弹框
 * @param Modal 弹框类
 * @param data 数据
...
export function showModal<T extends Modal>(Modal: new (...args: any) => T, data?: T["data"]) {
	return ModalManager.instance.show(Modal, data);
}
/**
...
export function showPromiseModal<T extends Modal>(Modal: new (...args: any) => T, data?: T["data"]) {
	const resModal = ModalManager.instance.show(Modal, data);
...
export const closeCurrentModal = () => {
	ModalManager.instance.closeCurrent();
};
...
export function changeScene<T extends Scene>(scene: new (...args: any) => T, data?: T["data"]) {
	SceneManager.instance.change(scene, data);
}
